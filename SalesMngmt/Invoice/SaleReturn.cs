...
namespace SalesMngmt.Invoice
{
    public partial class SaleReturn : Form
    {
        ...
        public SaleReturn SInvoice { get; set; }
        public SaleReturn(int Company)
        {
            InitializeComponent();
        ...
        private void SaveRecord(String InvoiceNo)
        {
            ...
            try
            {
                if (InvoiceNo == "0")
                {
                    ...
                    sale.InvoiceNO = txtInvoice.Text;
              ...
              //  sale.TransportExpense = Convert.ToDouble(txtTransportExpense.Text.DefaultZero());
                ...
                foreach (DataGridViewRow row in invGrid.Rows)
                {
                    if (row.Cells[0].Value != null)
                    {
                        ...
                        var saleM = db.Sales_M.AsNoTracking().Where(x => x.InvNo == txtInvoice.Text).FirstOrDefault();
                        var item = db.Sales_D.Where(x => x.IID == id && x.RID==saleM.RID).FirstOrDefault();
                        var item1 = db.Items.AsNoTracking().Where(X => X.IID == id).FirstOrDefault();
                        SalesR_D detail = new SalesR_D();
                        detail.RID = sale.RID;
                        ...
                        if (checkstock.Inv_YN == false)
                        {
                            ...
                            ledger.SRJ = ctn + Convert.ToDouble(row.Cells[3].Value);
                            ...
                            //  gl.MOP_ID = 2;
                            gl.Qty_IN = ctn + Convert.ToDouble(row.Cells[3].Value);
                            ...
                            gl.Debit = 0;
                            gl.CompID = compID;
                            gl.Credit = Convert.ToDouble(row.Cells[8].Value.DefaultZero());
                            db.GL.Add(gl);
                            db.SaveChanges();
                            ...
                            //  gl.MOP_ID = 2;
                            gl1.Qty_IN = ctn + Convert.ToDouble(row.Cells[3].Value);
                            ...
                            gl1.Debit = Convert.ToDouble(item.PurPrice.DefaultZero()) * (ctn + Convert.ToDouble(row.Cells[3].Value));
                            gl1.Credit = 0;
                            gl1.CompID = compID;
                            db.GL.Add(gl1);
                            db.SaveChanges();
                            ...
                            gl2.Debit = Convert.ToDouble(row.Cells[8].Value.DefaultZero());
                            gl2.Credit = 0;
                            gl2.CompID = compID;
                            db.GL.Add(gl2);
                            db.SaveChanges();
                            ...
                            gl3.IPrice = item.PurPrice;
                            gl3.AC_Code = item1.AC_Code_Cost;
                            gl3.AC_Code2 = Convert.ToInt32(cmbxvendor.SelectedValue);
                            gl3.Narration = cmbxvendor.SelectedText;
                            //  gl.MOP_ID = 2;
                            gl3.Qty_IN = ctn + Convert.ToDouble(row.Cells[3].Value);
                            gl3.PAmt = Convert.ToDouble(item.PurPrice.DefaultZero()) * gl1.Qty_Out;//Convert.ToDouble(row.Cells[5].Value.DefaultZero());
                            ...
                            gl3.Debit = 0;
                            gl3.Credit = Convert.ToDouble(item.PurPrice.DefaultZero()) * gl1.Qty_Out; ;
                            gl3.CompID = compID;
                            db.GL.Add(gl3);
                            db.SaveChanges();
                        }
                        else
                        {
                            ...
                            gl1.Debit = Convert.ToDouble(row.Cells[8].Value.DefaultZero());
                            gl1.Credit = 0;
                            gl1.CompID = compID;
                            db.GL.Add(gl1);
                            db.SaveChanges();
        ...
        private void Items_Leave(Lib.Entity.Items item, MetroFramework.Controls.MetroTextBox txtbox = null)
        {
            ...
            else
            {
                try
                {
                    ...
                    var saleM = db.Sales_M.AsNoTracking().Where(x => x.InvNo == txtInvoice.Text && x.CompID == compID).FirstOrDefault();
                    ...
                    if (saleM == null)
                    {
                      ...
                      //  txtCode.Text = item.BarcodeNo;
                        ...
                        var saleD = db.Sales_D.AsNoTracking().Where(x => x.IID == item.IID && x.RID == saleM.RID).FirstOrDefault();
                        if (saleD != null)
                        {
                            cmbxItems.Text = item.IName;
                            txtCode.Text = item.BarcodeNo;
                            ...
                            txtpcs.Text = 1.ToString();
                            txtctn.Text = 0.ToString();
                            txtRate.Text = saleD.SalesPriceP.DefaultZero();
        ...
        private void cmbxItems_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            string Query = @"SELECT TOP 10 Items.IID , ISNULL(ArticleNo, '0') as Article, 
                           Items.IName as Product , ISNULL(SalesPrice,0) as Price , ISNULL(Color, 0) as Color, ISNULL(Size, 0) as Size
