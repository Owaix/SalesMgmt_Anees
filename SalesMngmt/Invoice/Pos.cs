        ...
        public void SilentPrint(Orders Obj, bool type)
        {
            try
            {
                ...
                foreach (var item in Obj.OrderDetailsModel)
                {
                    ...
                    var waiter = db.tbl_Employee.AsNoTracking().Where(x => x.ID == order.WaiterID && order.CompanyID== compy).FirstOrDefault();
                    if (waiter == null) {
                        orders.waiterName = "";
                    }
                    else { 
                    ...
                    DateTime dtCreatedDate = dtCreatdDate.Value;
              DataTable dt1= new DataTable();
                    DateTime startDate = dtCreatdDate.Value;
                    DateTime selectedDate = dtCreatdDate.Value.Date;

                    var orderCount = db.tbl_Order
                        .AsEnumerable()  // Forces evaluation in memory
                        .Where(s => s.OrderDate.Date == selectedDate && s.CompanyID == compy)
                        .Count();

                    var orders123 = db.tbl_Order
    .Where(s => DbFunctions.TruncateTime(s.OrderDate) == selectedDate && s.CompanyID == compy)
    .ToList();
                    //  dt1 = ReportsController.orderNO(dtCreatdDate.Value, compy);
                    //int value= ReportsController.GetOrderCount(dtCreatdDate.Value, compy);
                    //    int count = db.tbl_Order.Count(x => DbFunctions.TruncateTime(x.OrderDate) == DbFunctions.TruncateTime(dtCreatedDate)
                    //&& x.CompanyID == compy);

                    //order.OrderNo = (Convert.ToInt32( dt.Columns["0"][0]) + 1).ToString();
